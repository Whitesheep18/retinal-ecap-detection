    ...
    def generate(self, num_stimuli=1,verbose=0):
        ...
        for i in range(num_stimuli):
            ...
            for j in range(num_cells):
                ...
                start_idx = int(np.random.exponential(self.spike_train_start_lambda_ms) / 1000 * self.fs)
                AP_indexes.append(start_idx)
...
if __name__ == "__main__":
    ...
    while len(spikes) < 1000:
        segments, AP_start_idx, data = rec.generate(1, verbose=0)
        print(AP_start_idx)
        noised_data = rec.add_white_noise(data, SNR_dB=20)
        noised_data = rec.add_mains_electricity_noise(noised_data, SNR_dB=20)
        noised_data = rec.add_spontaneous_spikes(noised_data, firing_Hz=1000)

        for idx in AP_start_idx:
            if idx + 200 <= len(noised_data):
                spikes.append(noised_data[idx: idx + 200])
            
            if len(spikes) >= 1000:
                break
        
        print(f"Collected {len(spikes)} spikes")

    # spikes_array = np.array(spikes)
    # np.save('spike_vector_1000.npy', spikes_array)


    # plain = []
    # while len(plain) < 1000:
    #     segments, _, data = rec.generate(1, verbose=0)
    #     random_numbers = np.random.randint(0, 2800, 50)
    #     noised_data = rec.add_white_noise(data, SNR_dB=20)
    #     noised_data = rec.add_mains_electricity_noise(noised_data, SNR_dB=20)
    #     noised_data = rec.add_spontaneous_spikes(noised_data, firing_Hz=1000)
    ...
    #         if idx + 200 <= len(noised_data):
    #             plain.append(noised_data[idx: idx + 200])
            
    #         if len(plain) >= 1000:
    #             break
        
    #     print(f"Collected {len(plain)} plain")

    # plain_array = np.array(plain)
    # np.save('plain_vector_1000.npy', plain_array)


    # pred_data = []

    # while len(pred_data) < 1000:
    #     segments, _, data = rec.generate(1, verbose=0)
    #     random_numbers = np.random.randint(0, 2800, 50)
    #     noised_data = rec.add_white_noise(data, SNR_dB=20)
    #     noised_data = rec.add_mains_electricity_noise(noised_data, SNR_dB=20)
    ...
    #     for idx in random_numbers:
    #         if idx + 200 <= len(noised_data):
    #             pred_data.append(noised_data[idx: idx + 200])
            
    #         if len(pred_data) >= 1000:
    #             break
        
    #     print(f"Collected {len(pred_data)} pred_data")

    # pred_data_array = np.array(pred_data)
    # np.save('pred_data_vector_1000.npy', pred_data_array)
