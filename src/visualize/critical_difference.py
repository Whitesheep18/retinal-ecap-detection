...
def multiple_wilcoxon(alpha=0.05, method='fdr', df_perf=None):
    ...
    # count the number of tested datasets per regressor
    df_counts = pd.DataFrame({'count': df_perf.groupby(
        ['Model']).size()}).reset_index()
    # get the maximum number of tested datasets
    max_nb_datasets = df_counts['count'].max()
    # get the list of regressors who have been tested on nb_max_datasets
    regressors = list(df_counts.loc[df_counts['count'] == max_nb_datasets]
                       ['Model'])
    # test the null hypothesis using friedman before doing a post-hoc analysis
    friedman_p_value = friedmanchisquare(*(
        np.array(df_perf.loc[df_perf['Model'] == c]['RMSE test'])
        for c in regressors))[1]
    if friedman_p_value >= alpha:
        # then the null hypothesis over the entire regressors cannot be rejected
        print('the null hypothesis over the entire regressors cannot be rejected')
        exit()
    ...
    for i in range(m - 1):
        # get the name of regressor one
        regressor_1 = regressors[i]
        # get the performance of regressor one
        perf_1 = np.array(df_perf.loc[df_perf['Model'] == regressor_1]['RMSE test']
                          , dtype=np.float64)
        for j in range(i + 1, m):
            # get the name of the second regressor
            regressor_2 = regressors[j]
            # get the performance of regressor one
            perf_2 = np.array(df_perf.loc[df_perf['Model'] == regressor_2]
                              ['RMSE test'], dtype=np.float64)
    ...
    # get the rank data
    rank_data = np.array(sorted_df_perf['RMSE test']).reshape(m, max_nb_datasets)

    # create the data frame containg the metrics
    df_ranks = pd.DataFrame(data=rank_data, index=np.sort(regressors), columns=
